...
class ExportRulesUseCase:
    ...
    def prepare_export(self, tenant_id: str) -> tuple[list[dict], dict]:
        ...
        self.rule_service.save_rendered_rules_for_tenant(tenant, artifact_rules)
    ...
    def _build_summary(self, tenant, mapped_rules: list[dict]) -> dict:
        ...
        artifact_root = (
            f"artifacts/{tenant.tenant_id}/{tenant.siem_id}"
            if tenant.siem_id
            else f"artifacts/{tenant.tenant_id}/_default"
        )
